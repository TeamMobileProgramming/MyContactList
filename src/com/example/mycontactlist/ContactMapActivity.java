			...
			public void onMapReady(final GoogleMap gMap) {
				...
				if (contacts.size() > 0) {
					...
					for (int i = 0; i < contacts.size(); i++) {
						...
						googleMap.addMarker(new MarkerOptions().position(point).title(currentContact.getContactName())
								.snippet(address));
					}
					googleMap.animateCamera(
							CameraUpdateFactory.newLatLngBounds(builder.build(), measuredWidth, measuredHeight, 100));
				} else {
